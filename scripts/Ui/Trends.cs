...
{
    public static Func<(string Key, Func<Control> Body)?> People(History past) => One(
        "population", "Население за год", Fmt.Count, "людей", Skin.People, past, History.Line.People);

    public static Func<(string Key, Func<Control> Body)?> Workers(History past) => One(
        "workers", "Рабочая сила за год", Fmt.Count, "рук", Skin.Labour, past, History.Line.Workers);

    public static Func<(string Key, Func<Control> Body)?> Employment(History past) => One(
        "employment", "Занятость за год", value => Fmt.Percent(value), "занятость", Skin.Labour,
        past, History.Line.Employment);

    public static Func<(string Key, Func<Control> Body)?> Load(History past) => One(
        "load", "Загрузка за год", value => Fmt.Percent(value), "загрузка", Skin.Plants,
        past, History.Line.Load);

    public static Func<(string Key, Func<Control> Body)?> Output(History past) => One(
        "output", "ВВП за год", Fmt.Cash, "ВВП", Skin.Output, past, History.Line.Gdp);

    public static Func<(string Key, Func<Control> Body)?> Plants(History past) => One(
        "plants", "Предприятия за год", Fmt.Count, "предприятий", Skin.Plants, past, History.Line.Plants);

    public static Func<(string Key, Func<Control> Body)?> Treasury(History past) => One(
    ...
    public static Func<(string Key, Func<Control> Body)?> Rate(History past) => One(
    ...
    public static Func<(string Key, Func<Control> Body)?> Inflation(History past) => () => TrendCard.Of(
    ...
    public static Func<(string Key, Func<Control> Body)?> Price(History past, Func<GoodType> good) => () =>
        TrendCard.Of($"price:{good()}", $"{Names.Of(good())}: цена за год", Fmt.Cash,
            () => [new Trace("цена", Names.ColourOf(good()).Lightened(0.2f),
                past.PricesOf(good(), History.Year))]);

    /// <summary>Выпуск и заказ товара за год одним графиком: врозь по ним не видно, идёт
    /// ли склад в плюс или в минус.</summary>
    public static Func<(string Key, Func<Control> Body)?> Flow(History past, Func<GoodType> good) => () =>
    ...
    public static Func<(string Key, Func<Control> Body)?> ToWorld(History past, Func<GoodType> good) => () =>
    ...
    private static Func<(string Key, Func<Control> Body)?> One(
        string key,
        string title,
