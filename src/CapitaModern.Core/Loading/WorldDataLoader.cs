...
public static class WorldDataLoader
{
    ...
    /// <remarks>
    /// Вдвое выше нормы <see cref="Prices.TargetCoverDays"/>: мир должен начинать партию там,
    /// где заводам не нужно ни добивать склад, ни разгружать его, а равновесие, к которому
    /// ведут правило полки и порча залежи, выше самой нормы.
    ///
    /// Ровно норму пробовали. Тогда первый год уходил на то, чтобы добить полки: склады мира
    /// утраивались с 8.4 до 22.7 трлн, ко второму году забивались, загрузку срезало — и это
    /// был единственный провал ВВП за сорок лет. А прежде, до порчи залежи, восемьдесят суток
    /// раздували склады до 148 трлн — тогда их нечему было разгружать.
    /// </remarks>
