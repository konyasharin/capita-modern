...
public sealed class Simulation
{
    ...
    private void PlanBuilds()
    {
        foreach (var country in _world.Countries)
        {
            ...
            // Десятая доля капитала в год — столько в жизни и вкладывают валовыми вложениями.
            // Было пятая: пока компании были бедны, упора не было, а когда стройка начала
            // платить заводам, они разом бросались строить, выгребали материалы, и на втором
            // году ВВП проседал на 2.1%. С десятой — на 0.66%.
    ...
    private void Perish()
    {
        foreach (var country in _world.Countries)
        {
            ...
            // Хранение не бесплатно: товар портится, стареет и требует места. Доля малая, но
            // она и есть та обратная связь, которой избыток не переспорит.
            //
            // Без неё гора, набранная в ранние годы, не рассасывается никогда: у железной
            // руды к сороковому году лежало триста восемнадцать тысяч дней расхода — это
            // девятьсот лет, — а правило загрузки только глушило шахты до пяти процентов и
            // теряло на этом треть мировой мощности. Ни норма запаса, ни заявка на ввоз тут
            // не помогают: обе проверены и отвергнуты.
            foreach (var good in AllGoods)
            {
                if (good == GoodType.Services) continue;

                // Гниёт залежь, а не оборотный запас: что лежит сверх годового расхода. Товар,
                // которого никто не просит, не трогаем вовсе — его и так почти не делают, а
                // счёт выпуска в тестах должен сходиться до единицы.
                var flow = _norm.Get(country.Id, good);
                if (flow.Raw <= 0) continue;

                var sane = new GoodAmount(flow.Raw * SpoilAfterDays);
                var have = stock.Of(good);
                if (have <= sane) continue;

                var gone = new GoodAmount((have - sane).Raw * SpoilPerMyriad / 10_000);
                if (gone.Raw > 0) stock.TakeUpTo(good, gone);
            }
        }
    }

    /// <summary>Какая доля запаса пропадает за сутки, в десятитысячных.</summary>
    /// <remarks>
    /// Три десятитысячных — одиннадцать процентов за год, как и держать запас в жизни. При
    /// тысячной (треть в год) полки чинятся лучше, но мир сжигает по двенадцати процентов
    /// ВВП в год, и рост падает с 4.55% до 3.63%.
    /// </remarks>
    private const int SpoilPerMyriad = 3;

    /// <summary>Сколько суток расхода запас лежит, ничего не теряя.</summary>
    /// <remarks>
    /// Сорок — та же норма, по которой стоит цена: всё сверх неё уже не оборот, а залежь.
    /// Годом пробовал, и это почти ничего не даёт: разница между «всё сверх года» и «всё
    /// подряд» — как раз обычные запасы в сорок-триста дней, и облагать надо именно их. С
    /// годом рост падает до 3.26% и мир валится после двадцать девятого года.
    /// </remarks>
    private const int SpoilAfterDays = Prices.TargetCoverDays;
