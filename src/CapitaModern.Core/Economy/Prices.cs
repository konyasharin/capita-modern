...
public sealed class Prices
{
    ...
    /// </remarks>
    public const int MaxSwingTimes = 20;

    // Было сто. Настоящие эпизоды — это разы: нефть весной 2020 упала втрое, газ в Европе
    // 2021 вырос вшестеро, литий за год вдевятеро. Двадцатикратный размах покрывает их все
    // с запасом, а сотня давала цене уехать так далеко, что страна переставала покупать на
    // мировом рынке вовсе: её заявка стоила копейки. Мировой ВВП пятого года 65.3 → 67.2
    // трлн, цены на рельсах 5.7% → 13.9% — упираются чаще, зато уже не улетают.
