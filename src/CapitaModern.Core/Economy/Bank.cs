...
public sealed class Bank
{
    ...
    /// <summary>Во сколько раз выданное может превысить вклады. Денежный мультипликатор:
    /// выдавая кредит, банк создаёт вклад, и тот возвращается в банковскую систему.</summary>
    /// <remarks>
    /// Пока банк раздавал только принесённое, кредит упирался в вклады: у мира их было
    /// двадцать один триллион при розданных двадцати, и компаниям занимать было не у кого.
    /// Оттого и стояла стройка — семь миллионов отказов «нет денег» за партию.
    ///
    /// В жизни так и есть: кредит создаёт депозит, а не наоборот. Втрое — умеренная
    /// величина; в развитых хозяйствах отношение кредита к вкладам бывает и выше.
    /// </remarks>
    ...
    public Money Free
    {
        get
        {
            var usable = new Money(Deposits.Raw * (100 - Reserve) / 100 * Multiplier);
