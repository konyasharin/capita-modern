...
public sealed class CreditMarket
{
    ...
    /// <summary>За какой чертой нагрузки перестают давать вовсе, в процентах.</summary>
    /// <remarks>
    /// Заметно раньше черты отказа платить, и это важно. Раньше обе были четырьмястами: в
    /// тот же день, когда стране переставали давать в долг, ей становилось нечем
    /// перезанять — и она отказывалась. Отсюда был обрыв вместо склона: медиана нагрузки в
    /// момент отказа выходила ровно 401%, а минимум 400%.
    ///
    /// В жизни кредит закрывается задолго до отказа: зона долгового риска у МВФ начинается
    /// около двухсот сорока процентов вывоза, а отказываются платить куда позже и после
    /// долгих переговоров. Между двумя чертами у страны есть время урезать ввоз и выправить
    /// сальдо — то самое время, которого модель её лишала.
    /// </remarks>
    public const int SafeBurden = 240;

    /// <summary>За сколько лет заём гасится целиком. Средний срок государственного
    /// внешнего долга в мире около десяти лет: короче занимают только в кризис, дольше —
    /// у международных организаций.</summary>
    public const int LoanYears = 10;
